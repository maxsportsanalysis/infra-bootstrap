...
build {
  ...
  inline = [
    ...
    "# -------------------------",
    "# Create HTTP chainload script boot.ipxe with auto-install options",
    "# -------------------------",
    "cat << 'EOF' > /var/www/html/pxe/boot.ipxe",
    "#!ipxe",
    "echo 'Starting iPXE boot'",
    "dhcp",
    "set base-url http://${next-server}",
    "echo 'Booting Ubuntu 24.04 Server Installer...'",
    "kernel ${base-url}/pxe/ubuntu/24.04/vmlinuz ip=dhcp url=http://${next-server}/pxe/ubuntu/24.04/autoinstall.yaml console=ttyAMA0,115200n8",
    ...
    "# -------------------------",
    "# Configure dnsmasq for PXE boot (proxyDHCP)",
    "# -------------------------",
    ...
    "dhcp-boot=tag:ipxe,http://192.168.50.251/pxe/boot.ipxe",
    "EOF",
